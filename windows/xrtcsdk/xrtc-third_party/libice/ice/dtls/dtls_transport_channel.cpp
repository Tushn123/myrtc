...
DtlsTransportChannel::DtlsTransportChannel(IceTransportChannel* ice_channel) :
    ice_channel_(ice_channel)
{
    ice_channel_->SignalReadPacket.connect(this, &DtlsTransportChannel::OnReadPacket);
    ice_channel_->SignalWritableState.connect(this, &DtlsTransportChannel::OnWritableState);
    ice_channel_->SignalReceivingState.connect(this, &DtlsTransportChannel::OnReceivingState);

    webrtc::CryptoOptions crypto_options;
    srtp_ciphers_ = crypto_options.GetSupportedDtlsSrtpCryptoSuites();
}

DtlsTransportChannel::~DtlsTransportChannel() {

}

void DtlsTransportChannel::OnReadPacket(IceTransportChannel* /*channel*/,
...
void DtlsTransportChannel::OnWritableState(IceTransportChannel* channel) {
...
void DtlsTransportChannel::OnReceivingState(IceTransportChannel* channel) {
    set_receiving(channel->receiving());
}

void DtlsTransportChannel::set_receiving(bool receiving) {
...
bool DtlsTransportChannel::SetLocalCertificate(rtc::RTCCertificate* cert) {
    if (dtls_active_) {
...
bool DtlsTransportChannel::SetRemoteFingerprint(const std::string& digest_alg,
...
void DtlsTransportChannel::set_dtls_state(DtlsTransportState state) {
...
void DtlsTransportChannel::set_writable_state(bool writable) {
...
bool DtlsTransportChannel::SetupDtls() {
    ...
    dtls_->SignalEvent.connect(this, &DtlsTransportChannel::OnDtlsEvent);
    dtls_->SignalSSLHandshakeError.connect(this, 
            &DtlsTransportChannel::OnDtlsHandshakeError);
...
void DtlsTransportChannel::OnDtlsEvent(rtc::StreamInterface* /*dtls*/, int sig, int error) {
    if (sig & rtc::SE_OPEN) {
...
void DtlsTransportChannel::OnDtlsHandshakeError(rtc::SSLHandshakeError err) {
    RTC_LOG(LS_WARNING) << ToString() << ": DTLS handshake error=" << (int)err;
}

void DtlsTransportChannel::MaybeStartDtls() {
...
bool DtlsTransportChannel::HandleDtlsPacket(const char* data, size_t size) {
...
std::string DtlsTransportChannel::ToString() {
    std::stringstream ss;
...
bool DtlsTransportChannel::GetSrtpCryptoSuite(int* selected_crypto_suite) {
    if (dtls_state_ != DtlsTransportState::kConnected) {
        return false;
    }
...
bool DtlsTransportChannel::ExportKeyingMaterial(const std::string& label,
...
int DtlsTransportChannel::SendPacket(const char* data, size_t len) {
    if (ice_channel_) {
...
rtc::SSLRole DtlsTransportChannel::GetDtlsRole() const {
   return dtls_role_;
}

bool DtlsTransportChannel::SetDtlsRole(rtc::SSLRole role) {
    if (dtls_) {
        if (dtls_role_ != role) {
            RTC_LOG(LS_ERROR) << "SSL Role can't be reversed after the session is setup.";
            return false;
        }
        return true;
    }
