...
void ConnectionRequest::Prepare(StunMessage* msg) {
    ...
    if (IceRole::ICEROLE_CONTROLLING == connection_->port()->ice_role()) {
        msg->AddAttribute(std::make_unique<StunUInt64Attribute>(
            STUN_ATTR_ICE_CONTROLLING, 0));
    } else if (IceRole::ICEROLE_CONTROLLED == connection_->port()->ice_role()) {
        msg->AddAttribute(std::make_unique<StunUInt64Attribute>(
            STUN_ATTR_ICE_CONTROLLED, 0));
    }
    msg->AddAttribute(std::make_unique<StunByteStringAttribute>(
...
IceConnection::IceConnection(rtc::Thread* network_thread,
    UDPPort* port,
    const Candidate& remote_candidate) :
...
void IceConnection::MaybeUpdatePeerReflexiveCandidate(const Candidate& new_candidate)
{
...
void IceConnection::ReceivedPingResponse(int rtt) {
    ...
    if (rtt_samples_ > 0) {
        rtt_ = static_cast<int>(rtc::GetNextMovingAverage(rtt_, rtt, RTT_RATIO));
    } else {
        rtt_ = rtt;
    }
...
void DestoryCb(EventLoop* el, TimerWatcher* w, void* data) {
    ...
    IceConnection* conn = (IceConnection*)data;
    delete conn;
}
#endif

bool IceConnection::Dead(int64_t now) {
    if (last_received() > 0) {
...
bool IceConnection::TooManyPingFails(size_t max_pings, int rtt, int64_t now) {
    ...
    int64_t expected_response_time = pings_since_last_response_[max_pings-1].sent_time + rtt;
